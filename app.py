...
# Support ensembling 4 models by default. You can override with TFLITE_PATHS (comma-separated)
_default_models = [
    os.path.join(BASE_DIR, "STGCN_model(1).tflite"),
    os.path.join(BASE_DIR, "STGCN_model(2).tflite"),
    os.path.join(BASE_DIR, "STGCN_model(3).tflite"),
    os.path.join(BASE_DIR, "STGCN_model(4).tflite"),
]
...
def ensure_interpreters():
    global interpreters, input_details_list, output_details_list
    if interpreters:
        return
    if not MODEL_PATHS:
...
def predict():
    try:
        ...
        # Run all models: each returns top-3; average (sum/4) the probabilities
        num_models = len(interpreters)
        agg_probs = None
        top3_by_model = []
        for itp, inp_det, out_det in zip(interpreters, input_details_list, output_details_list):
            ...
            itp.invoke()
            probs = itp.get_tensor(out_det[0]['index'])[0]  # (num_classes,)
            # track top-3
        ...
        # average
        avg_probs = agg_probs / float(num_models)
